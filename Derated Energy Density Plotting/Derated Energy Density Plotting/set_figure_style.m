function set_figure_style(scale)
    ...
    set(l,'FontSize',12*scale,'FontName','TimesNewRoman','Interpreter','latex')    

    % Change line width
     h = findobj(gcf,'type','line');
     set(h,'linewidth',1*scale);
    ...
    for index = 1:length(ha_all)
        ha = ha_all(index);
        % Axis tick label
        set(ha,'FontSize',9*scale) 
        set(ha,'FontName','TimesNewRoman','TickLabelInterpreter','latex') % axis font

        % Axis title label
        set(ha.YLabel,'FontSize',11*scale,'Interpreter','latex');
        set(ha.XLabel,'FontSize',11*scale,'Interpreter','latex');
